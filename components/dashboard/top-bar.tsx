...
import { Search, Menu, ShieldCheck, LogOut, User as UserIcon, Settings, X, Bell } from "lucide-react"
...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-2 md:gap-4 border-b border-border/40 bg-background/60 backdrop-blur-xl px-4 md:px-6 lg:px-10 shadow-sm transition-all">
      {showMobileSearch ? (
        <div className="flex w-full items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="relative flex-1">
            ...
            <Input
              ...
              type="search"
              placeholder="Buscar..."
              className="pl-10 h-10 bg-background border-primary/20 ring-1 ring-primary/20 rounded-xl"
            />
          </div>
          <Button
            variant="ghost"
        ...
        <>
          <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
            <Button variant="ghost" size="icon" className="md:hidden text-foreground hover:bg-muted h-10 w-10 shrink-0" onClick={onMenuClick}>
          ...
          <div className="flex items-center gap-1 md:gap-4">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="md:hidden text-muted-foreground hover:text-primary h-10 w-10 shrink-0"
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="gap-2 md:gap-3 h-10 md:h-12 pl-1 md:pl-2 pr-2 md:pr-4 rounded-xl md:rounded-2xl hover:bg-muted border border-border/40 bg-muted/30 transition-all group max-w-[150px] md:max-w-none">
                  ...
                  <div className="flex flex-col items-start text-left hidden sm:flex overflow-hidden">
                    <span className="text-xs md:text-sm font-bold leading-none text-foreground truncate w-full">{user.firstName}</span>
              ...
              <DropdownMenuContent align="end" className="w-56 md:w-64 p-2 bg-popover/95 backdrop-blur-xl border-border/40 shadow-3xl rounded-xl md:rounded-2xl">
                <DropdownMenuLabel className="p-3 md:p-4 pt-2">
                  <div className="flex flex-col gap-1">
                    <p className="font-extrabold text-base md:text-lg leading-tight text-foreground truncate">{user.fullName}</p>
